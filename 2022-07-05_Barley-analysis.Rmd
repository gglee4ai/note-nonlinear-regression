...
```{r}
plot(intervals(fit.lis), layout = c(3, 1), lcol = "black", par.strip.text = list(cex = 2), col = "black")
```

```{r}
## This first graph suggest that we do need random effects for theta 1 and theta 2
## but the random effect for lrc might not be needed

plot(fit.lis, year ~ resid(.), abline = 0)
```

```{r}
## This plot shows a much better spread for the residuals than the single non-linear
## equation fitted to all of the data. This is naturally expected.
```
...
```{r}
## The standardized residuals allows us to see the outliers for individual years
## Notice that they are within 2

## The problem with this approach is that we are using (19 x 3 = 57) 57 parameters
## to model these data and we are not taking advantage of the similarity among curves
## additionally we don't have an estimate of how much these parameters vary among years
...
```{r}
## This is an interesting plot because it shows the variability in the random effects
## by year. It would be interesting to add a plot where the years are sorted to detect
## a possible trend.
